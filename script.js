...
const paginationAboveContainer = document.querySelector(
	"#pagination-above-container"
);
const paginationBelowContainer = document.querySelector(
...
function renderSite(data) {
	...
	while (paginationAboveContainer.firstChild)
		paginationAboveContainer.firstChild.remove();
	while (paginationBelowContainer.firstChild)
		paginationBelowContainer.firstChild.remove();
...
function generateURL() {
	...
	if (!selectedGenres.length) {
		...
		if (selectedPlatform) paramPlatform();
		if (selectedGenres.length)
			parameters.push(`tag=${selectedGenres.join(".")}`);
		if (sortFilter.value) paramSort();
